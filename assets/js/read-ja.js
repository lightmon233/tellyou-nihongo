(function () {
  ...
  // Pre-load the voices and limit speech controls to ja-note shortcodes.
  window.speechSynthesis.getVoices();
  document.querySelectorAll('.ja-note__ja').forEach((it) => {
    injectReadButton(it)
  })
})()

function injectReadButton(tag) {
  const jaText = genJaText(tag)
  if (!jaText.match(/[ァ-ヶぁ-ん一-龯々ー]+/)) {
    return
  }
  ...
  // The SVG is kept as a template literal so its quotes stay readable.
  ...
  tag.insertAdjacentElement('beforeend', btn);
}

function genJaText(tag) {
  const clone = tag.cloneNode(true)
  clone.querySelectorAll('button, rt').forEach((it) => it.remove())
  return clone.textContent.replaceAll(/（[^）]+）/g, '').trim()
}
...
function addVoiceHandler(btn, jaText) {
    ...
    const pitch = Number('{{ .Site.Params.voice.pitch }}');
    const rate = Number('{{ .Site.Params.voice.rate }}');
    ...
    const sentences = splitTextForSpeech(jaText).filter((sentence) => sentence.trim());
