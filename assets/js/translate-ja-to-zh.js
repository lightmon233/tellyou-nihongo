...
(function () {
  let notes = document.querySelectorAll('.ja-note__ja');
  for (let i = 0; i < notes.length; i++) {
    let noteJARow = notes[i]
    let jaText = getNoteJAText(noteJARow);
    ...
    btn.addEventListener('click', function () {
      ...
      let result = noteJARow.parentElement.querySelector('.ja-note__machine-translation');
      if (!result) {
        result = document.createElement('blockquote');
        result.className = 'ja-note__machine-translation'
        noteJARow.parentElement.insertAdjacentElement('beforeend', result)
      }

      let div3 = document.createElement('div');
      div3.classList.add('t-animated-background')
      div3.innerText = '翻译中...'
      result.replaceChildren(div3)
      ...
      translateJAToZH(jaText).then(function (response) {
        ...
        btn.disabled = false
        btn.classList.remove('doflip');
      })

      //reset animation
      btn.classList.remove('doflip');
      btn.classList.add('doflip');
      setTimeout(function () {
        btn.classList.remove('doflip');
      }, 5000);
    }, false)
    noteJARow.insertAdjacentElement('beforeend', btn);
  }
})()

function getNoteJAText(tag) {
